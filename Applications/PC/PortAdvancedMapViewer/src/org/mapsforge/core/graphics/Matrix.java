...
public class Matrix {
	...
	/**
	 * Creates a Matrix object from the float array. The array becomes the
	...
	private void addTransform(float[] matrix) {
		...
		// first row
		tmp[0] = matrix[0] * mValues[0] + matrix[1] * mValues[3] + matrix[2]
				* mValues[6];
		tmp[1] = matrix[0] * mValues[1] + matrix[1] * mValues[4] + matrix[2]
				* mValues[7];
		tmp[2] = matrix[0] * mValues[2] + matrix[1] * mValues[5] + matrix[2]
				* mValues[8];

		// 2nd row
		tmp[3] = matrix[3] * mValues[0] + matrix[4] * mValues[3] + matrix[5]
				* mValues[6];
		tmp[4] = matrix[3] * mValues[1] + matrix[4] * mValues[4] + matrix[5]
				* mValues[7];
		tmp[5] = matrix[3] * mValues[2] + matrix[4] * mValues[5] + matrix[5]
				* mValues[8];

		// 3rd row
		tmp[6] = matrix[6] * mValues[0] + matrix[7] * mValues[3] + matrix[8]
				* mValues[6];
		tmp[7] = matrix[6] * mValues[1] + matrix[7] * mValues[4] + matrix[8]
				* mValues[7];
		tmp[8] = matrix[6] * mValues[2] + matrix[7] * mValues[5] + matrix[8]
				* mValues[8];
	...
	public void setSinCos(float sinValue, float cosValue, float px, float py) {
		...
		// scale
		addTransform(new float[] { cosValue, -sinValue, 0, sinValue, cosValue,
				0, 0, 0, 1 });
		// translate back the pivot
		addTransform(new float[] { 1, 0, px, 0, 1, py, 0, 0, 1 });
	}
	 ...
	 * rectangle to the destination rectangle, returning true if the result can
	 * be represented.
	 * 
	 * @param src
	 ...
	 */
	/*
	 * public boolean setRectToRect(RectF src, RectF dst, ScaleToFit stf) { if
	 * (dst == null || src == null) { throw new NullPointerException(); }
	 * 
	 * if (src.isEmpty()) { reset(); return false; }
	 * 
	 * if (dst.isEmpty()) { mValues[0] = mValues[1] = mValues[2] = mValues[3] =
	 * mValues[4] = mValues[5] = mValues[6] = mValues[7] = 0; mValues[8] = 1; }
	 * else { float tx, sx = dst.width() / src.width(); float ty, sy =
	 * dst.height() / src.height(); boolean xLarger = false;
	 * 
	 * if (stf != ScaleToFit.FILL) { if (sx > sy) { xLarger = true; sx = sy; }
	 * else { sy = sx; } }
	 * 
	 * tx = dst.left - src.left * sx; ty = dst.top - src.top * sy; if (stf ==
	 * ScaleToFit.CENTER || stf == ScaleToFit.END) { float diff;
	 * 
	 * if (xLarger) { diff = dst.width() - src.width() * sy; } else { diff =
	 ...
	 * 
	 * mValues[0] = sx; mValues[4] = sy; mValues[2] = tx; mValues[5] = ty;
	 * mValues[1] = mValues[3] = mValues[6] = mValues[7] = 0;
	 * 
	 * } // shared cleanup mValues[8] = 1; return true; }
	 */

	/*
	 * public boolean setRectToRect(RectF src, RectF dst,
	 * MatrixOriginal.ScaleToFit stf) { throw new
	 * UnsupportedOperationException("CALL TO PARENT FORBIDDEN"); }
	 */
	...
	protected static void checkPointArrays(float[] src, int srcIndex,
	...
	public void mapPoints(float[] dst, int dstIndex, float[] src, int srcIndex,
		...
		for (int i = 0; i < pointCount; i++) {
			...
			// vars
			float x = mValues[0] * src[i + srcIndex] + mValues[1]
					* src[i + srcIndex + 1] + mValues[2];
			float y = mValues[3] * src[i + srcIndex] + mValues[4]
					* src[i + srcIndex + 1] + mValues[5];
	 ...
	 */
	/*
	 * public boolean mapRect(RectF dst, RectF src) { if (dst == null || src ==
	 * null) { throw new NullPointerException(); }
	 * 
	 * // array with 4 corners float[] corners = new float[] { src.left,
	 * src.top, src.right, src.top, src.right, src.bottom, src.left, src.bottom,
	 * };
	 * 
	 * // apply the transform to them. mapPoints(corners);
	 * 
	 * // now put the result in the rect. We take the min/max of Xs and min/max
	 * of Ys dst.left = Math.min(Math.min(corners[0], corners[2]),
	 * Math.min(corners[4], corners[6])); dst.right =
	 * Math.max(Math.max(corners[0], corners[2]), Math.max(corners[4],
	 * corners[6]));
	 * 
	 * dst.top = Math.min(Math.min(corners[1], corners[3]), Math.min(corners[5],
	 * corners[7])); dst.bottom = Math.max(Math.max(corners[1], corners[3]),
	...
	private final static int kTranslate_Mask = 0x01; // !< set if the matrix has
														// translation
	private final static int kScale_Mask = 0x02; // !< set if the matrix has X
													// or Y scale
	private final static int kAffine_Mask = 0x04; // !< set if the matrix skews
													// or rotates
	private final static int kPerspective_Mask = 0x08; // !< set if the matrix
														// is in perspective
	...
	@SuppressWarnings("unused")
	private final static int kAllMasks = kTranslate_Mask | kScale_Mask
			| kAffine_Mask | kPerspective_Mask | kRectStaysRect_Mask;

	// these guys align with the masks, so we can compute a mask from a variable
	// 0/1
	@SuppressWarnings("unused")
