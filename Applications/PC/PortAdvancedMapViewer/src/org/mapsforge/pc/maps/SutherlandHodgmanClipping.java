...
final class SutherlandHodgmanClipping {
		...
		for (int i = 0; i < polyline.length - 2; i += 2) {
			...
			if (isStartPointInside) {
				...
				if (isEndPointInside) {
					...
					float[] intersection = computeIntersection(edge, x1, y1,
							x2, y2);
	...
	private static float[] computeIntersection(int[] edge, double x1,
			double y1, double x2, double y2) {
		if (edge[1] == edge[3]) {
			// horizontal edge
			return new float[] {
					(float) (x1 + (edge[1] - y1) * ((x2 - x1) / (y2 - y1))),
					edge[1] };
		}
		// vertical edge
		return new float[] { edge[0],
				(float) (y1 + (edge[0] - x1) * ((y2 - y1) / (x2 - x1))) };
	}
	...
	static float[] clipPolyline(float[] polyline, int[] rectangle) {
		// bottom edge
		float[] clippedPolyline = clipPolylineToEdge(polyline, new int[] {
				rectangle[0], rectangle[1], rectangle[2], rectangle[1] });
		// right edge
		clippedPolyline = clipPolylineToEdge(clippedPolyline, new int[] {
				rectangle[2], rectangle[1], rectangle[2], rectangle[3] });
		// top edge
		clippedPolyline = clipPolylineToEdge(clippedPolyline, new int[] {
				rectangle[2], rectangle[3], rectangle[0], rectangle[3] });
		// left edge
		return clipPolylineToEdge(clippedPolyline, new int[] { rectangle[0],
