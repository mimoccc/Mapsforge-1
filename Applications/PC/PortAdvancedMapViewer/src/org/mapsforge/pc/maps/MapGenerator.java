...
/**
 * A MapGenerator provides map images. This abstract base class handles all
 * thread specific actions and provides the queue for jobs, which need to be
 * processed and scheduled.
 */
abstract class MapGenerator extends Thread {
	private static final GeoPoint DEFAULT_START_POINT = new GeoPoint(51.33,
			10.45);
	...
	public final void run() {
		...
		this.currentTileBitmap = Bitmap.createBitmap(Tile.TILE_SIZE,
		...
		while (!isInterrupted()) {
			...
			if (!this.tileRAMCache.containsKey(this.currentMapGeneratorJob)
					&& !this.tileMemoryCardCache
							.containsKey(this.currentMapGeneratorJob)) {
				...
				if (executeJob(this.currentMapGeneratorJob)) {
					...
					if (this.mapView != null) {
						...
						this.mapView.putTileOnBitmap(
								this.currentMapGeneratorJob,
								this.currentTileBitmap);
						this.mapView.postInvalidate();
					}

					// put the tile image in the cache
					System.out.println("MapGenerator (Run): Put in Cache: " + this.currentMapGeneratorJob);
...
//							this.currentTileBitmap);
			...
			if (!isInterrupted() && this.jobQueue1.isEmpty()
					&& this.requestMoreJobs && this.mapView != null) {
	...
	private void schedule() {
		if (this.mapView != null) {
			while (!this.jobQueue1.isEmpty()) {
				this.jobQueue2.offer(this.mapView.setJobPriority(this.jobQueue1
						.poll()));
			}
	...
	/**
	 * Adds the given job to the queue. A call to this method has no effect if
	 * the given job is already in the queue.
	 * 
	...
	/**
	 * This method will by called at the end of the run method when the thread
	 * was interrupted. It can be used to clean up objects and to close any open
	 * connections.
	 */
	...
	/**
	 * Returns the default starting point on the map. May be overridden by
	 * subclasses.
	 * 
	...
	/**
	 * Returns the default zoom level of the map. May be overridden by
	 * subclasses.
	 * 
	...
	/**
	 * Returns the name of the MapGenerator. It will be used as the name for the
	 * thread.
	 * 
	...
	/**
	 * This method is called each time the MapView gets attached to the window.
	 * May be overridden by subclasses to react on this event.
	 * <p>
	...
	/**
	 * This method is called each time the MapView gets detached from the
	 * window. May be overridden by subclasses to react on this event.
	 * <p>
	 * The default implementation of this method does nothing.
	 */
	...
	/**
	 * This method is called each time before a tile needs to be processed. It
	 * can be used to clear any data structures that will be needed when the
	 * next map tile will be processed.
	 */
	 ...
	 *            true if the MapGenerator may ask for more jobs, false
	 *            otherwise.
	 */
	 ...
	 */
	final void setTileCaches(TileRAMCache tileRAMCache,
	...
	/**
	 * This method is called only once before any map tile is requested. It can
	 * be used to set up data structures or connections that will be needed.
	 * 
	 * @param bitmap
