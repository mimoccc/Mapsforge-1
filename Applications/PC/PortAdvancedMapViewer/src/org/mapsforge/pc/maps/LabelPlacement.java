...
class LabelPlacement {
	...
	static class ReferencePosition {
		...
		ReferencePosition(float x, float y, int nodeNumber, float width,
				float height, SymbolContainer symbol) {
			this.x = x;
			this.y = y;
	...
	private void centerLabels2(ArrayList<PointTextContainer> labels) {
		for (int i = 0; i < labels.size(); i++) {
			...
			if (this.label.symbol != null) {
				this.label.y = this.label.y
						- this.label.symbol.symbol.getHeight() / 2 - 3;
			}
		}
	}
	...
	private ArrayList<PointTextContainer> processFourPointGreedy(
		...
		// position
		PriorityQueue<ReferencePosition> priorUp = new PriorityQueue<ReferencePosition>(
		...
		for (int z = 0; z < labels.size(); z++) {
			if (labels.get(z) != null) {
				if (labels.get(z).symbol != null) {
					...
					// up
					refPos[z * 4] = new ReferencePosition(tmp.x
							- tmp.boundary.width() / 2, tmp.y
							- tmp.symbol.symbol.getHeight() / 2 - dis, z,
							tmp.boundary.width(), tmp.boundary.height(),
							tmp.symbol);
					// down
					refPos[z * 4 + 1] = new ReferencePosition(tmp.x
							- tmp.boundary.width() / 2, tmp.y
							+ tmp.symbol.symbol.getHeight() / 2
							+ tmp.boundary.height() + dis, z,
							tmp.boundary.width(), tmp.boundary.height(),
							tmp.symbol);
					// left
					refPos[z * 4 + 2] = new ReferencePosition(tmp.x
							- tmp.symbol.symbol.getWidth() / 2
							- tmp.boundary.width() - dis, tmp.y
							+ tmp.boundary.height() / 2, z,
							tmp.boundary.width(), tmp.boundary.height(),
							tmp.symbol);
					// right
					refPos[z * 4 + 3] = new ReferencePosition(tmp.x
							+ tmp.symbol.symbol.getWidth() / 2 + dis, tmp.y
							+ tmp.boundary.height() / 2 - 0.1f, z,
							tmp.boundary.width(), tmp.boundary.height(),
							tmp.symbol);
				} else {
					refPos[z * 4] = new ReferencePosition(labels.get(z).x
							- ((labels.get(z).boundary.width()) / 2),
							labels.get(z).y, z, labels.get(z).boundary.width(),
							labels.get(z).boundary.height(), null);
					refPos[z * 4 + 1] = null;
					refPos[z * 4 + 2] = null;
					refPos[z * 4 + 3] = null;
				}
		...
		while (priorUp.size() != 0) {
			...
			resolutionSet.add(new PointTextContainer(this.label.text,
					this.reference.x, this.reference.y, this.label.paintFront,
					this.label.paintBack, this.label.symbol));
			...
			while (priorDown.size() != 0) {
				if (priorDown.peek().x < this.reference.x
			...
			for (int i = 0; i < linkedRef.size(); i++) {
				if ((linkedRef.get(i).x <= this.reference.x
						+ this.reference.width)
						&& (linkedRef.get(i).y >= this.reference.y
								- linkedRef.get(i).height)
						&& (linkedRef.get(i).y <= this.reference.y
								+ linkedRef.get(i).height)) {
	...
	/**
	 * This method uses an adapted greedy strategy for the fixed two position
	 * model, above and under. It uses no priority search tree, because it will
	 * not function with symbols only with points. Instead it uses two minimum
	 * heaps. They work similar to a sweep line algorithm but have not a O(n log
	 * n +k) runtime. To find the rectangle that has the leftest edge, I use
	 * also a minimum Heap. The rectangles are sorted by their x coordinates.
	 * 
	 * @param labels
	 *            label positions and text
	 * @param symbols
	 ...
	 *            area label positions and text
	 * @return list of labels without overlaps with symbols and other labels by
	 *         the two fixed position greedy strategy
	 */
	private ArrayList<PointTextContainer> processTwoPointGreedy(
		...
		for (int z = 0; z < labels.size(); z++) {
			...
			if (this.label.symbol != null) {
				refPos[z * 2] = new ReferencePosition(this.label.x
						- (this.label.boundary.width() / 2) - 0.1f,
						this.label.y - this.label.boundary.height()
								- this.startDistanceToSymbols, z,
						this.label.boundary.width(),
						this.label.boundary.height(), this.label.symbol);
				refPos[z * 2 + 1] = new ReferencePosition(this.label.x
						- (this.label.boundary.width() / 2), this.label.y
						+ this.label.symbol.symbol.getHeight()
						+ this.startDistanceToSymbols, z,
						this.label.boundary.width(),
						this.label.boundary.height(), this.label.symbol);
			} else {
				refPos[z * 2] = new ReferencePosition(this.label.x
						- (this.label.boundary.width() / 2) - 0.1f,
						this.label.y, z, this.label.boundary.width(),
						this.label.boundary.height(), null);
				refPos[z * 2 + 1] = null;
			}
		}
		...
		while (priorRight.size() != 0) {
			...
			resolutionSet.add(new PointTextContainer(this.label.text,
					this.reference.x, this.reference.y, this.label.paintFront,
					this.label.paintBack, this.reference.symbol));

			// Removes the other position that is a possible position for the
			// label of one point
			// of interest
			...
			// find overlapping labels and deletes the reference points and
			// delete them
			LinkedList<ReferencePosition> linkedRef = new LinkedList<ReferencePosition>();

			while (priorLeft.size() != 0) {
				if (priorLeft.peek().x < this.reference.x
						+ this.reference.width) {
	...
	private void removeEmptySymbolReferences(
			ArrayList<PointTextContainer> nodes,
			ArrayList<SymbolContainer> symbols) {
	...
	private void removeNonValidateReferencePosition(ReferencePosition[] refPos,
		...
		for (int i = 0; i < symbols.size(); i++) {
			...
			// this.rect1 = new android.graphics.Rect((int) this.smb.x - dis,
			// (int) this.smb.y
			...
			for (int y = 0; y < refPos.length; y++) {
				if (refPos[y] != null) {
					...
					this.rect2 = new Rect((int) refPos[y].x,
							...
							(int) (refPos[y].x + refPos[y].width),
							(int) (refPos[y].y));

					// if (android.graphics.Rect.intersects(this.rect2,
					// this.rect1)) {
		...
		for (PointTextContainer areaLabel : areaLabels) {
			...
			// this.rect1 = new android.graphics.Rect((int) areaLabel.x - dis,
			// (int) areaLabel.y
	 ...
	 */
	private void removeOutOfTileAreaLabels(
			ArrayList<PointTextContainer> areaLabels) {
	 ...
	 *            area labels from the actual tile
	 */
	private void removeOverlappingAreaLabels(
			ArrayList<PointTextContainer> areaLabels) {
		...
		for (int x = 0; x < areaLabels.size(); x++) {
			...
			this.rect1 = new Rect((int) this.label.x - dis, (int) this.label.y
					- dis, (int) (this.label.x + this.label.boundary.width())
					+ dis,
			...
			for (int y = x + 1; y < areaLabels.size(); y++) {
				if (y != x) {
					...
					// this.rect2 = new android.graphics.Rect((int)
					// this.label.x,
					...
					// if (android.graphics.Rect.intersects(this.rect1,
					// this.rect2)) {
	 ...
	 *            list of labels
	 */
	private void removeOverlappingSymbolsWithAreaLabels(
			ArrayList<SymbolContainer> symbols,
		...
		for (int x = 0; x < pTC.size(); x++) {
			...
			for (int y = 0; y < symbols.size(); y++) {
				...
				// this.rect2 = new android.graphics.Rect((int) this.smb.x,
				// (int) this.smb.y,
	...
	/**
	 * The inputs are all the label and symbol objects of the current tile. The
	 * output is overlap free label and symbol placement with the greedy
	 ...
	 */
	ArrayList<PointTextContainer> placeLabels(
			ArrayList<PointTextContainer> labels,
			ArrayList<SymbolContainer> symbols,
			ArrayList<PointTextContainer> areaLabels, Tile cT) {
		...
		if (!DEFAULT) {
			...
			this.dependencyCache.removeOverlappingObjectsWithDependencyOnTile(
					returnLabels, areaLabels, symbols);

			if (returnLabels.size() != 0) {
				switch (this.placementOption) {
				case 0:
					returnLabels = processTwoPointGreedy(returnLabels, symbols,
							areaLabels);
					break;
				case 1:
					returnLabels = processFourPointGreedy(returnLabels,
							symbols, areaLabels);
					break;
				default:
					break;
			...
			this.dependencyCache.fillDependencyOnTile(returnLabels, symbols,
					areaLabels);
		} else {
			centerLabels(areaLabels);
