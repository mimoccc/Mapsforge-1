...
class Rtree2DIndexPage<T> extends AbstractHilbertRtreePage<T, Rect> {
	...
	Rtree2DIndexPage(Storage storage, Object[] objs, Rect[] rects) {
		super(storage, objs, rects);
	}

	Rtree2DIndexPage(Storage storage, Object obj, Rect rect) {
		super(storage, obj, rect);
	}

	Rtree2DIndexPage(Storage storage, AbstractHilbertRtreePage<T, Rect> root) {
		super(storage, root);
	}

	Rtree2DIndexPage(Storage storage) {
		super(storage);
	}
	...
	AbstractHilbertRtreePage<T, Rect> newRoot(Storage storage,
			AbstractHilbertRtreePage<T, Rect> oldRoot) {
		return new Rtree2DIndexPage<T>(storage, oldRoot);
	}

	@Override
	AbstractHilbertRtreePage<T, Rect> newNode(Storage storage,
			AbstractHilbertRtreePage<T, Rect> parentNode) {
		Rtree2DIndexPage<T> node = new Rtree2DIndexPage<T>(storage);
