...
import org.mapsforge.preprocessing.map.MapElementWay;
import org.mapsforge.preprocessing.map.MercatorProjection;
import org.mapsforge.preprocessing.map.Tile;
import org.mapsforge.preprocessing.map.Utils;


class Utils {
	private static GeometryFactory geoFac = new GeometryFactory();
	private static final int[] tileBitMaskValues = new int[] { 32768, 16384,
			2048, 1024, 8192, 4096, 512, 256, 128, 64, 8, 4, 32, 16, 2, 1 };
	 ...
	 * @param zoom  the zoom level at which the bounding box should be calculated.
	...
	static LinearRing getBoundingBox(long tileX, long tileY, byte zoom) {
		...
		return geoFac.createLinearRing(new Coordinate[] {
				new Coordinate(maxLat, minLon), new Coordinate(minLat, minLon),
				new Coordinate(minLat, maxLon), new Coordinate(maxLat, maxLon),
				new Coordinate(maxLat, minLon) });
	}

	/**
	 * Returns all subtiles of a given tile on a configurable higher zoom level.
	 * The resulting list is computed by recursively calling getSubtiles() on
	 * all four subtiles of the given tile. On each recursive call the second
	 * parameter is decremented by one until zero is reached.
	 * 
	 * @param currentTile the starting tile for the computation of the subtiles.
	 * @param zoomLevelDistance how many zoom levels higher the subtiles should be.
	 * @return a list containing all subtiles on the higher zoom level.
	 * @throws InvalidParameterException if {@code zoomLevelDistance} is less than zero.
	 */
	static List<Tile> getSubtiles(Tile currentTile, byte zoomLevelDistance) {
		...
		if (zoomLevelDistance == 0) {
			...
			childrenTiles.addAll(getSubtiles(new Tile(currentTile.x * 2,
					currentTile.y * 2, (byte) (currentTile.zoomLevel + 1)),
	...
	/**
	 * Calculates for each way that is related to a given tile a tile bitmask.
	 * The bitmask determines for which tiles on zoom level initialTile.zoom+2 
	 * the way is needed for rendering.
	 * 
	 * @param initialTile a tile for which all subtiles on zoom level initialTile.zoom+2 
	 * 			will be calculated 
	 * @param ways all ways which are related to the given tile
	...
	static Map<MapElementWay, Short> getTileBitmasksForWays(Tile initialTile,
		...
		for (int i = 0; i < childrenTiles.size(); i++) {
			...
			Geometry tileBBox = geoFac.createPolygon(getBoundingBox(tmpTile.x,
					tmpTile.y, tmpTile.zoomLevel), null);
			...
			for (Entry<MapElementWay, Geometry> entry : entries) {
				...
				if (way.crosses(tileBBox) || tileBBox.contains(way)
						|| way.intersects(tileBBox)) {
					//add a specified value to the tile bitmask of the way and put it in the result map
					result.put(currentElement, (currentElement.tileBitmask |= tileBitMaskValues[i]));
				}
			}
		}
	 ...
	 * 
	 * @param way the way
	 * @param wayNodes a list of coordinates of the way nodes
	 * @param wayType the type of the way, 1 = simple way, 2 = closed way/area, 3 =
	 *            part of a multipolygon
	 * @param zoom the zoom level at which the tiles should be calculated
	 * @return a set of tiles which contain the whole way or a part of the way
	 * 
	 */
	static Set<Tile> wayToTiles(MapElementWay way, Coordinate[] wayNodes,
			int wayType, byte zoom){
		Geometry geoWay;
		...
		if (wayType == 1) {
			...
			geoWay = geoFac.createPolygon(geoFac.createLinearRing(wayNodes),
					null);
			way.convexness = (int) ((geoWay.getArea() / geoWay.convexHull()
					.getArea())*100);
		}
		...
		for (long i = minTileX; i <= maxTileX; i++) {
			for (long j = minTileY; j <= maxTileY; j++) {
				tiles.put(new Coordinate(i, j), geoFac.createPolygon(Utils
						.getBoundingBox(i, j, zoom), null));
			}
		}
		...
		for (Entry<Coordinate, Geometry> e : set) {
			...
			if (wayType != 1) {
				if (currentTile.within(geoWay)
						|| currentTile.intersects(geoWay)
