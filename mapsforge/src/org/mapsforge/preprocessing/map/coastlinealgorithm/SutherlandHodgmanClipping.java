 ...
 */
package org.mapsforge.preprocessing.map.coastlinealgorithm;

/**
 * Implementation of the Sutherland-Hodgman algorithm for clipping polygons.
 */
final class SutherlandHodgmanClipping {
	private static float[] clipPolylineToEdge(float[] polyline, int[] edge) {
		...
		for (int i = 0; i < polyline.length - 2; i += 2) {
			...
			boolean isStartPointInside = isInside(x1, y1, edge);
			boolean isEndPointInside = isInside(x2, y2, edge);
